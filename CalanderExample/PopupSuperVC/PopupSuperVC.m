...
-(void)presentViewControllerForViewController:(UIViewController *)viewConteroler{
    ...
    if (IsPhone()) {
        ...
        [viewConteroler presentViewController:testVC animated:false completion:nil];
        ...
        popup.sourceView = viewConteroler.view;
        popup.delegate = self;
...
// For Background View Method
-(void)presentViewControllerForviewConteroller:(UIViewController *) objView sourceView:(UIView *)sourceView ArrowDirection:(UIPopoverArrowDirection)arrowDirection isBackground:(BOOL)background {
    
    [self presentViewControllerForviewConteroller:objView sourceView:sourceView ArrowDirection:arrowDirection InputViews:nil isBackground:background];
}
-(void)presentViewControllerForviewConteroller:(UIViewController *) objView sourceView:(UIView *)sourceView ArrowDirection:(UIPopoverArrowDirection)arrowDirection InputViews:(NSArray<UIView *> *)passthroughViews isBackground:(BOOL)background {
    if (IsPhone()) {
        [self iPhonePVCSetting:objView];
    }
    else{
...
-(void)presentViewControllerForviewConteroller:(UIViewController *) objView sourceView:(UIView *)sourceView InputViews:(NSArray<UIView *> *)passthroughViews atPostion:(CGRect)sourceRect{
   ...
   // [self iPadAtPositionPVCSetting:popup atPostion:sourceRect];
    [self iPadAtPositionPVCSetting:popup atPostion:sourceRect withSourceView:objView.view];
...
-(void)presentVCForRightSide:(UIViewController *)objView WithInputView:(UIView *)sourceView {
    ...
    if (IsPhone()) {
        ...
        sourceRect.origin.y = (objView.view.frame.size.height - self.sizePreferredContentSize.height)/2;
...
//    [self.view.layer setShadowRadius:9.0];
    ...
    [objView presentViewController:heplerVCiPhone animated:false completion:^{
        [UIView animateWithDuration:0.1 animations:^{
            self->heplerVCiPhone.view.alpha = 1.0f;
        }];
    }];
...
-(void)iPadAtPositionPVCSetting:(UIPopoverPresentationController *)popup  atPostion:(CGRect)sourceRect withSourceView:(UIView *)view{
    popup.permittedArrowDirections = (UIPopoverArrowDirection)NULL;
    popup.sourceView = view;
    popup.sourceRect = sourceRect;
}
...
-(void)hidePopupIphoneView{
    if (self.closePopupVC != nil) {
        self.closePopupVC(self);
    }
    else {
        [self popoverPresentationControllerShouldDismissPopover];
    }
